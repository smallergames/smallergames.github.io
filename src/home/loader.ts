...
export const LOADER_LETTERS = [
  { id: "w", label: "W" },
  { id: "i", label: "I" },
  { id: "p", label: "P" },
] as const;

export const LOADER_SEQUENCE = ["w", "i-right", "p", "i-left"] as const;

export type LoaderSequenceStep = (typeof LOADER_SEQUENCE)[number];
export type LoaderLetterId = (typeof LOADER_LETTERS)[number]["id"];
...
export type GlitchPatternState = {
...
export type TimingControllerState = {
  stepTiming: StepTiming;
...
export const REST_ANIMATION = { rotate: 0, x: 0, scale: 1 };
...
function getInitialGlitchPattern(): GlitchPatternState {
  return {
    ...INITIAL_GLITCH_PATTERN,
  };
}

export function createInitialTimingController(): TimingControllerState {
  const initialRoll = getStepTiming(LOADER_SEQUENCE[0], getInitialGlitchPattern());
...
export function getReducedMotionTimingController(): TimingControllerState {
  return {
    stepTiming: { ...BASE_STEP_TIMING[LOADER_SEQUENCE[0]], ...NO_GLITCH },
    glitchPattern: getInitialGlitchPattern(),
  };
}

export function getNextSequenceIndex(sequenceIndex: number) {
  return (sequenceIndex + 1) % LOADER_SEQUENCE.length;
}

export function getNextTimingController(
  step: LoaderSequenceStep,
  pattern: GlitchPatternState,
): TimingControllerState {
  const nextRoll = getStepTiming(step, pattern);
...
export function getLetterMotion(
  step: LoaderSequenceStep,
  letterId: LoaderLetterId,
  timing: StepTiming,
) {
